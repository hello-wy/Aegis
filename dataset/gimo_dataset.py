...
class EgoEvalDataset(data.Dataset):
    def __init__(self, config, train=False):
        # folder = "SLICES_888s"
        config.sample_points = 32768
        folder = config.folder
        # config.sample_points = int(folder.split('_')[-1])
        ...
        self.occupancy_map = torch.load(os.path.join(self.dataroot, "318_4096", split, "occ_map.pth"))
        
        self.scenes = torch.load(os.path.join(self.dataroot, "315_32768", split,
    ...
    def __getitem__(self, index):
        ...
        # Clone cached tensors so online augmentation does not corrupt the dataset in-place.
        gazes = self.gazes[index].clone()
        poses_input = self.poses_input[index].clone()
        poses_label = self.poses_label[index].clone()
        joints_input = self.joints_input[index].clone()
        joints_label = self.joints_label[index].clone()
        scene_points = self.scenes[index].clone()
        occupancy_map = self.occupancy_map[index].clone()
        # dense_scene_points = self.dense_scenes[index]
        ...
        if self.train and self.data_augmentation:
            ...
            scene_points += np.random.normal(loc=0, scale=0.01, size=scene_points.shape)
            scene_points = torch.from_numpy(scene_points).float()
            
            gazes_np = gazes.numpy().reshape(-1, 3)
            gazes = torch.from_numpy((random_rotation @ gazes_np.T).T).float().unsqueeze(1)

            for i in range(self.input_seq_len):
                ori_s = Rotation.from_rotvec(poses_input[i][:3].numpy()).as_matrix()
                trans_s = poses_input[i][3:6].numpy()

                ori_s = Rotation.from_matrix(random_rotation @ ori_s).as_rotvec()
                trans_s = random_rotation @ trans_s
                
                poses_input[i][:3] = torch.from_numpy(ori_s).float()
                poses_input[i][3:6] = torch.from_numpy(trans_s).float()

                joints_input_np = joints_input[i].numpy()  # [J, 3]
                joints_input[i] = torch.from_numpy((random_rotation @ joints_input_np.T).T).float()
            
            for f in range(self.output_seq_len):
                ori_s = Rotation.from_rotvec(poses_label[f][:3].numpy()).as_matrix()
                trans_s = poses_label[f][3:6].numpy()
                
                ori_s = Rotation.from_matrix(random_rotation @ ori_s).as_rotvec()
                trans_s = random_rotation @ trans_s
                                
                # 更新poses_label
                poses_label[f][:3] = torch.from_numpy(ori_s).float()
                poses_label[f][3:6] = torch.from_numpy(trans_s).float()
