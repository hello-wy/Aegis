...
def scene_to_local(scene_glob, trans, root_orient, rot_rep="aa"):
    """
    Convert global scene points to local coordinate system.
    Origin is trans.
    Orientation is aligned such that +X is the character's forward direction.
    (Assuming SMPL canonical is +Z forward, +Y up).
    
    :param scene_glob (*, N, 3)
    :param trans (*, 3)
    :param root_orient (*, 3, 3) or (*, 6) etc
    :param rot_rep str
    """
    R_global = convert_rotation(root_orient, rot_rep, "mat")
    centered_points = scene_glob - trans.unsqueeze(-2)
    
    # x_local = (x_global - t) @ R
    local_scene = torch.bmm(centered_points, R_global)
    
    return local_scene

def rot_scene(scene_data, ori):
    """
    Rotate scene points according to given orientation.
    :param scene_data: B,N,3
    :param ori: B,3
    """
    R = convert_rotation(ori, 'aa', 'mat')  # B,3,3
    scene_rotated = torch.einsum("bnj,bij->bni", scene_data, R)
    return scene_rotated